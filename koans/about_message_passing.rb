...
class AboutMessagePassing < Neo::Koan
  ...
  def test_send_with_underscores_will_also_send_messages
    ...
    #
    # Answer:
    # Use __send__ on unknown objects, because they may have overridden send:
  ...
  # on: What is send actually for? What is a "message" in this context?
  # Answer: send is for calling a method, this is useful to do dynamically.
  class Car
  ...
  def test_send_calls_methods

    c = Car.new
    c.send("make=", "Honda")

    assert_equal "Honda", c.make

    h = { 'model' => "CRV", 'year' => 2014 }
    h.each do |key, value|
        c.send("#{key}=", value)
    end

    assert_equal "CRV", c.model
    assert_equal 2014, c.year

    assert_equal :vroom, c.send('drive')
    assert_equal :vroom, c.send(:drive)
  end
  ...
  def test_calling_method_missing_causes_the_no_method_error
    ...
    exception = assert_raise(NoMethodError) do
      typical.method_missing(:foobaz)
    end
    assert_match(/foobaz/, exception.message)
  end
